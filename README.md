...
スマホでは開始後に「記録」「発行済み・精算待ち」「履歴」を画面下の固定タブから切り替えます。使用頻度の低い「勤務報告」は、画面上部の日付欄にある小さなリンクから開けます。PCでは従来どおり上部の4タブを使用します。駐車番号ボタンは実際の駐車場配置に合わせ、左側1〜8番・右側21〜9番（11番なし）の2列で表示します。開始後のヘッダーに表示される店舗名をタップすると、その日の店舗を変更できます。
